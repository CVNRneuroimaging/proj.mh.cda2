...
_For timekeeping audits: the two "OMT regressor" tasks below are verification tasks that only produced one file (`IG_omt317s02_seq3124.txt`), but the commands and output of those verification steps are included in these notes, and were discussed in an email exchange that spanned 2:45p - 4:30p_
